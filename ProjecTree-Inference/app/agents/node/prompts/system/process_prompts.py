...
[지시사항]
1. 에픽 이름은 '명사형'으로 간결하게 작성하세요. (예: [회원] 소셜 로그인 통합). **'주니어를 위한', '교육용' 등의 사족은 이름에 절대 포함하지 마세요.**
2. 명세(Description)는 비즈니스 배경과 범위를 명확히 하여 개발자들이 길을 잃지 않게 하세요. (700token 이내)
3. [Description 검증 및 출력 절차 - 엄수]
   당신은 반드시 다음의 **[생각 -> 도구 호출 -> 결과 확인 -> 최종 답변]** 과정을 순서대로 수행해야 합니다. 바로 최종 JSON을 출력하지 마세요.

   - **STEP 1 (생각):** 상세 명세(Description)의 초안을 마음속으로 작성합니다.
   - **STEP 2 (검증):** 작성한 초안을 `validate_description(description='...')` 도구에 전달하여 실행합니다.
   - **STEP 3 (확인):** 도구의 반환값을 확인합니다.
     - "❌ 검증 실패"가 뜨면, 피드백을 반영하여 내용을 수정하고 **다시 STEP 2로 돌아갑니다.** (통과할 때까지 반복)
     - "✅ 검증 통과"가 뜨면, STEP 4로 이동합니다.
   - **STEP 4 (최종 답변):** **반드시** 도구 검증을 통과한 description 텍스트를 그대로 사용하여 최종 결과물(Structured Output)을 생성합니다.

   ⚠️ **주의: `validate_description` 도구를 호출하지 않고 바로 결과를 반환하면 작업이 실패한 것으로 간주됩니다.**
...
[지시사항]
1. 태스크 이름은 '동사형'으로 기술적인 행동이 드러나게 작성하세요. (예: API 구현, 스키마 설계). **'주니어를 위한' 등의 문구는 이름에서 제외하고 깔끔하게 작성하세요.**
