...
2. 명세(Description)는 비즈니스 배경과 범위를 명확히 하여 개발자들이 길을 잃지 않게 하세요.
3. **summary는 반드시 60자 이내**로 핵심 키워드만 담아 작성하세요. (예: '소셜 로그인 및 프로필 관리 구현')
...
2. 명세(Description)에는 반드시 **인수 조건(Acceptance Criteria)**이 포함되어야 합니다.
3. **summary는 반드시 60자 이내**로 핵심 키워드만 담아 작성하세요. (예: '이메일 회원가입 기능 구현')
...
4. **summary는 반드시 60자 이내**로 핵심 키워드만 담아 작성하세요. (예: '회원가입 API 구현')
...
3. **summary는 반드시 60자 이내**로 핵심 키워드만 담아 작성하세요. (예: 'N+1 쿼리 최적화')
