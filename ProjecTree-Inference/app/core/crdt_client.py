...
class CRDTClient:
    ...
    def __init__(self, timeout: float = 30.0):
        ...
        """
        self.base_url = f"{settings.CRDT_SERVER_URL}{settings.CRDT_SERVER_PATH}"
    ...
    async def send(self, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"CRDT 요청 전송: {self.base_url}, body: {body}")
            ...
            logger.error(f"CRDT 서버 HTTP 오류: {e.response.status_code} - {e.response.text}")
            return None
