...
class ChatLog(Base, TimestampMixin):
    ...
    id = Column(BigInteger, primary_key=True, index=True)  # Identity generated by default
    chat_room_id = Column(UUID(as_uuid=True), ForeignKey("chat_room.id"), nullable=False)
...
class FileProperty(Base, TimestampMixin):
    ...
    uploader_id = Column(BigInteger, nullable=True) # Assuming references member.id but no FK constraint in schema?
    ...
    document_property = relationship("DocumentProperty", uselist=False, back_populates="file_property")
...
class Member(Base, TimestampMixin):
    ...
    nodes = relationship("Node", back_populates="owner") # user_id in Node references Member
...
class Workspace(Base, TimestampMixin):
    ...
    start_date = Column(TIMESTAMP(timezone=True)) # timestamp(6) usually implies microsecond precision
    end_date = Column(TIMESTAMP(timezone=True))

    # Relationships
    documents = relationship("DocumentProperty", back_populates="workspace")
    teams = relationship("Team", back_populates="workspace")
    function_specifications = relationship("FunctionSpecification", back_populates="workspace")
    workspace_tech_stacks = relationship("WorkspaceTechStack", back_populates="workspace")
...
class TechVocabulary(Base, TimestampMixin):
    ...
    node_tech_stack = relationship("NodeTechStack", back_populates="tech_vocabulary", uselist=False)
    workspace_tech_stack = relationship("WorkspaceTechStack", back_populates="tech_vocabulary", uselist=False)
...
class TechStackInfo(Base, TimestampMixin):
    ...
    # Relationships
    node_tech_stack = relationship("NodeTechStack", back_populates="tech_stack_info", uselist=False)
...
class WorkspaceTechStack(Base, TimestampMixin):
    ...
    id = Column(BigInteger, primary_key=True, index=True)
    tech_vocab_id = Column(BigInteger, ForeignKey("tech_vocabulary.id"), nullable=False, unique=True)
    workspace_id = Column(BigInteger, ForeignKey("workspace.id"), nullable=False)

    # Relationships
    tech_vocabulary = relationship("TechVocabulary", back_populates="workspace_tech_stack")
...
class NodeTechStack(Base, TimestampMixin):
    ...
    tech_stack_info_id = Column(BigInteger, ForeignKey("tech_stack_info.id"), unique=True)
...
class Node(Base, TimestampMixin):
    ...
    node_type = Column(String(31), nullable=False) # Discriminator
    ...
    tech_stacks = relationship("NodeTechStack", back_populates="node") # Assuming one-to-many or many-to-one? Schema says node_tech_stack has node_id
    ...
    candidates = relationship("Candidate", foreign_keys="[Candidate.node_id]", back_populates="node")
    
    # Derivation node relationship in Candidate
    # Schema: candidate.derivation_node_id -> node.id (FK)
    derived_candidates = relationship("Candidate", foreign_keys="[Candidate.derivation_node_id]", back_populates="derivation_node")
...
class ProjectNode(Node):
    ...
    __mapper_args__ = {
        "polymorphic_identity": "PROJECT_NODE",
    }
...
class Candidate(Base, TimestampMixin):
    ...
    derivation_node = relationship("Node", foreign_keys=[derivation_node_id], back_populates="derived_candidates")
