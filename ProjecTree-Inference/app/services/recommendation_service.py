...
class RecommendationService:
    ...
    async def recommend_tech_stack(
        ...
        """
        logger.info(f"[RecommendationService] 기술 스택 추천 시작 - node_id: {request.node_id}")
        
        try:
            # 노드 정보 조회
            node = self.node_repository.get_by_id(db, request.node_id)
            logger.debug(f"[RecommendationService] 노드 조회 완료 - node_type: {node.node_type}, name: {node.name}")
            ...
            if existing_node_tech_stacks:
                ...
                logger.debug(f"[RecommendationService] 제외할 기술 스택: {excluded_tech_names}")
            ...
            logger.debug(f"[RecommendationService] 추천된 기술 수: {len(tech_list)}")
            ...
            logger.error(f"[RecommendationService] 기술 스택 추천 실패 - node_id: {request.node_id}")
            logger.error(f"[RecommendationService] Error Type: {type(e).__name__}")
            logger.error(f"[RecommendationService] Error Message: {str(e)}")
            logger.error(f"[RecommendationService] Traceback:\n{traceback.format_exc()}")
            raise