...
import { GitBranch, Sparkles, Users, Layers } from 'lucide-react';
...
const features = [
  {
    ...
    title: '트리 시각화',
    description:
      '프로젝트 전체 구조를 한눈에 파악하세요. 복잡한 기능 계층도 직관적인 트리 뷰로 쉽게 탐색할 수 있습니다.',
    size: 'large',
  },
  {
    icon: Sparkles,
    title: 'AI 기술 추천',
    description:
      '프로젝트 요구사항에 딱 맞는 최적의 기술 스택을 AI가 추천해 드립니다.',
    size: 'small',
  },
  {
    icon: Layers,
    title: '스마트 명세서',
    description: '기능 트리에서 상세 명세서를 자동으로 생성하세요.',
    size: 'small',
  },
  {
    icon: Users,
    title: '실시간 협업',
    description:
      '라이브 커서와 동시 편집, 실시간 음성 채팅과 채팅 기능을 제공합니다. 실시간 커뮤니케이션과 동시 편집으로 팀원들과 함께 프로젝트를 만들어가세요.',
    size: 'medium',
  },
];
  ...
  return (
    ...
    >
      ...
      <div className={`relative z-10 p-6 ${feature.size === 'large' ? 'p-8' : ''}`}>
        ...
        >
          <Icon className="h-6 w-6 text-emerald-600 transition-all duration-300 group-hover:text-emerald-500" />
        </div>

        <h3
          className={`mb-2 font-semibold text-zinc-900 ${feature.size === 'large' ? 'text-2xl' : 'text-lg'}`}
        >
          {feature.title}
        </h3>
        <p
          className={`text-zinc-500 leading-relaxed font-medium ${feature.size === 'large' ? 'text-base' : 'text-sm'}`}
        >
          {feature.description}
        </p>
        ...
        {feature.size === 'large' && (
          <div className="mt-6 grid grid-cols-2 gap-3">
            {['드래그 앤 드롭', '실시간 동기화', '줌 컨트롤', '인피니트 캔버스'].map(
              (tag) => (
                <div
                  key={tag}
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        >
          ...
          <p className="mx-auto max-w-2xl text-zinc-500 text-lg font-medium">
            기획부터 개발 착수까지의 워크플로우를 혁신적으로 단축하는 강력한
            기능들
          </p>
        </motion.div>
