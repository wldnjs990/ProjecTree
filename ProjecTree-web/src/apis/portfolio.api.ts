 ...
 * - 없으면 404 에러 반환
 */
export async function getPortfolio(workspaceId: number): Promise<Portfolio | null> {
    try {
 ...
 * - 생성 시간이 오래 걸릴 수 있음
 */
export async function generatePortfolio(workspaceId: number): Promise<Portfolio> {
 ...
 * - { id, content } 전송
 */
export async function updatePortfolio(id: number, content: string): Promise<Portfolio> {
