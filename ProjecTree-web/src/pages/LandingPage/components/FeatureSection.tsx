...
function BentoCard({
    ...
    return (
        <motion.div
            ...
            className={`group relative overflow-hidden rounded-2xl border border-zinc-800/50 bg-zinc-900/40 backdrop-blur-md ${sizeClasses[feature.size as keyof typeof sizeClasses]}`}
        >
            {/* Spotlight Effect */}
            <div
            ...
            <div className={`relative z-10 p-6 ${feature.size === "large" ? "p-8" : ""}`}>
                {/* Icon with Glow */}
                <div
                    className="mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-950/50 border border-emerald-500/20 transition-all duration-300 group-hover:border-emerald-500/40"
                    style={{
                        boxShadow: isHovered ? "0 0 30px rgba(16, 185, 129, 0.3)" : "0 0 15px rgba(16, 185, 129, 0.1)",
                    }}
                >
                    <Icon
                        className="h-6 w-6 text-emerald-400 transition-all duration-300 group-hover:text-emerald-300"
                        style={{
                            filter: isHovered ? "drop-shadow(0 0 8px rgba(52, 211, 153, 0.8))" : "none",
                        }}
                    />
                </div>

                <h3 className={`mb-2 font-semibold text-white ${feature.size === "large" ? "text-2xl" : "text-lg"}`}>
                    {feature.title}
                </h3>
                <p className={`text-zinc-400 leading-relaxed ${feature.size === "large" ? "text-base" : "text-sm"}`}>
                    {feature.description}
                </p>
                ...
                {feature.size === "large" && (
                    <div className="mt-6 grid grid-cols-2 gap-3">
                        {["드래그 앤 드롭", "자동 레이아웃", "줌 컨트롤", "SVG 내보내기"].map((tag) => (
                            <div
                                key={tag}
                                className="rounded-lg border border-emerald-500/10 bg-emerald-950/30 px-3 py-2 text-xs text-emerald-300/70"
                            >
                                {tag}
                            </div>
                        ))}
    ...
    return (
        <section className="relative py-24 md:py-32 bg-zinc-950">
            ...
            <div className="absolute inset-0 overflow-hidden">
                <div className="absolute top-0 left-1/4 h-[500px] w-[500px] rounded-full bg-emerald-600/5 blur-[150px]" />
                <div className="absolute bottom-0 right-1/4 h-[400px] w-[400px] rounded-full bg-teal-500/5 blur-[120px]" />
            </div>
