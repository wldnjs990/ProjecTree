...
import type { FlowNode, FlowNodeData, FlowNodeType, YjsNode } from '../types/node';
import { flowNodeToYjsNode, yjsNodeToFlowNode } from '../utils/nodeTransform';

// Y.Map에 저장되는 노드 값 타입
type YNodeValue = FlowNodeType | string | undefined | { x: number; y: number } | FlowNodeData;
  ...
  const syncFromYjs = useCallback(() => {
    ...
    yNodesRef.current.forEach((yNode, nodeId) => {
      ...
      const yjsNode: YjsNode = {
        ...
        parentId: rawParentId === null ? undefined : (rawParentId as string | undefined),
