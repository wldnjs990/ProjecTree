...
export function EpicGroup({ epic, stories, onNodeClick, StoryGroupComponent }: EpicGroupProps) {
    ...
    <AccordionItem value={epic.id} className="border-b-0">
      ...
      >
        ...
        >
          <div className={cn("flex items-center gap-2", indentLevel.epic)}>
            ...
            <span className="text-xs font-medium px-1.5 py-0.5 rounded bg-violet-100 text-violet-700">에픽</span>
          </div>
      ...
      <AccordionContent className="pb-0">
        <Accordion type="multiple" defaultValue={[]} className="w-full">
          {stories.map((story) => (
            <StoryGroupComponent key={story.id} story={story} tasks={story.tasks} onNodeClick={onNodeClick} />
          ))}
        </Accordion>
      </AccordionContent>
    </AccordionItem>
