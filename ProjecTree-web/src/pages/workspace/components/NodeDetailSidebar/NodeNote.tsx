...
// 메모 편집 컴포넌트 (편집 모드)
export function NoteEdit({ value, onChange }: NoteEditProps) {
...
export function NoteDisplay({ note }: NoteDisplayProps) {
  ...
  return (
    <div className="rounded-[14px] border border-[rgba(227,228,235,0.5)] bg-[rgba(251,251,255,0.6)] backdrop-blur-sm overflow-hidden">
      ...
      {isExpanded && (
        <div className="px-4 pb-4">
          <div className="w-full min-h-[80px] p-3 text-sm text-[#0B0B0B] bg-[rgba(238,238,238,0.3)] rounded-lg whitespace-pre-wrap">
            {note || (
              <span className="text-[#9CA3AF]">메모가 없습니다.</span>
            )}
          </div>
        </div>
      )}
