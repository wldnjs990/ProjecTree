...
async def create_node(
    ...
    """
    logger.info(
        f"[Nodes API] 노드 생성 요청 시작 - candidate_id: {request.candidate_id}, parent_id: {request.parent_id}"
    )
    try:
        result = await service.create_node(db=db, request=request)
        logger.info(
            f"[Nodes API] 노드 생성 성공 - candidate_id: {request.candidate_id}, 생성된 node_id: {result.node_id}"
        )
        return result
    except ValueError as e:
        logger.warning(
            f"[Nodes API] 잘못된 요청 - candidate_id: {request.candidate_id}, error: {str(e)}"
        )
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        logger.error(
            f"[Nodes API] 노드 생성 중 오류 발생 - candidate_id: {request.candidate_id}, parent_id: {request.parent_id}"
        )
