...
async def generate_candidates(
    ...
    """
    logger.info(
        f"[Candidates API] 후보 노드 생성 요청 시작 - node_id: {request.node_id}, candidate_count: {request.candidate_count}"
    )
    try:
        result = await service.generate_candidates(
            db=db, request=request, workspace_id=request.workspace_id
        )
        logger.info(
            f"[Candidates API] 후보 노드 생성 성공 - node_id: {request.node_id}, 생성된 후보 수: {len(result.candidates)}"
        )
        return result
    except NotImplementedError as e:
        logger.warning(
            f"[Candidates API] 서비스 미구현 - node_id: {request.node_id}, error: {str(e)}"
        )
        raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail=str(e))
    except Exception as e:
        logger.error(
            f"[Candidates API] 후보 노드 생성 중 오류 발생 - node_id: {request.node_id}"
        )
