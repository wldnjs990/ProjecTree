...
3. **[중요] Summary 필드 작성법**:
   ...
   - 당신이 N개의 후보를 생성한다면, `validate_summary` 도구도 **반드시 N번 호출**해야 합니다.
   - ❌ **절대 금지**: 한 번 호출한 결과를 모든 항목에 복사해서 붙여넣기 하지 마세요.
   - ✅ **필수 행동**:
     1. 첫 번째 후보 생성 -> 설명(description) 작성 -> 도구 호출(Input: 첫 번째 설명) -> 결과 반영
     2. 두 번째 후보 생성 -> 설명(description) 작성 -> 도구 호출(Input: 두 번째 설명) -> 결과 반영
   - 각 후보의 `description`이 다르므로, `summary` 결과도 당연히 달라야 합니다.
### 예시 (출력 포맷 참고용)
[
    {
        "name": "[회원 시스템] 회원 관리",
